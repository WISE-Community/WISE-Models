...
function beforeSaveNodeStateListener(type, args, obj) {
	...
	if(stepObjectNodeId == nodeId) {
		...
		stepObject.node.isCompleted = function (nodeVisits){
			console.log(nodeVisits);
			if (typeof nodeVisits !== "undefined" && typeof nodeVisits.length !== "undefined" && nodeVisits.length > 0 && typeof nodeVisits[nodeVisits.length-1].nodeStates !== "undefined"){
				var nodeStates = nodeVisits[nodeVisits.length-1].nodeStates;
				var prevNodeState = nodeStates[nodeStates.length-1];
				var score = prevNodeState.score;
				var rubricIndex = prevNodeState.rubricIndex;
				var rubricCount = prevNodeState.rubricCount;
				var setCompletedMinScore = typeof prevNodeState.setCompletedMinScore !== "undefined" ? prevNodeState.setCompletedMinScore : null;

				/* 
				*	We may have locked this step. In default cases we unlock the screen.
				*	However, if rubric has "unlockScreenOnScore" numerical property
				*   we check whether the score has been reached and only unluckScreen if score is met.
				*   If this property is set to null we cannot unlock screen on this rubric (we wait for a later one)
				*   Therefore if there are three rubrics to meet, set null to unluckScreenOnScore for first 2 and use a score for the third.
				*   BE CAREFUL WITH THIS PROPERTY, YOU DON'T WANT TO LOCK A STUDENT IN PERMANENTLY
				*/
				if (setCompletedMinScore == null){
					return true;
				} else if (score >= setCompletedMinScore && rubricIndex >= (rubricCount-1)){
					return true;
				} else {
					return false;
				}
