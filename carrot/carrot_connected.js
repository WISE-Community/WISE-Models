...
// the latest component state that was saved for the current visit
var latestComponentStateSaved = null;

// latest work from this component
var latestStudentWorkFromThisComponent = null;

// latest work from other components in this node
var latestStudentWorkFromThisNode = null;

// latest work from other specific components
var latestStudentWorkFromOtherComponents = null;

/*
 * whether to always create a new component state or reuse the component state
 * that was created for the current visit
 */
var reuseComponentState = true;
...
function start() {
    ...
    if (isNewTrial) {
        ...
        // get the work from the previous step's open response component
        var previousStepOpenResponseComponentState = getStudentWorkByNodeIdAndComponentId('node54', '1b630z8wa5');

        // get the work from the open response component
        var openResponseComponentState = getStudentWorkByNodeIdAndComponentId('node53', 'yo6qlvcg3u');
        
        // get the work from the multiple choice component
        var multipleChoiceComponentState = getStudentWorkByNodeIdAndComponentId('node53', '17qww5dxhs');
        
        var otherWork = '';

        var previousStepOpenResponseResponse = '';
        
        if (previousStepOpenResponseComponentState != null) {
            var studentData = previousStepOpenResponseComponentState.studentData;

            if (studentData != null) {
                // get the text that the student typed into the previous step
                var response = studentData.response;
                previousStepOpenResponseResponse += response;
            }
        }

        otherWork += 'Open Response from previous step: ';

        if (previousStepOpenResponseResponse == '') {
            otherWork += 'Empty';
        } else {
            otherWork += previousStepOpenResponseResponse;
        }

        otherWork += '<br/>';

        var currentStepOpenResponseResponse = '';
        ...
        if (openResponseComponentState != null) {
            ...
            if (studentData != null) {
                ...
                var response = studentData.response;
                currentStepOpenResponseResponse += response;
            }
        }

        otherWork += 'Open Response from current step: ';

        if (currentStepOpenResponseResponse == '') {
            otherWork += 'Empty';
        } else {
            otherWork += currentStepOpenResponseResponse;
        }

        otherWork += '<br/>';

        var currentStepMultipleChoiceResponse = '';
                ...
                if (studentChoices != null) {
                    ...
                    for (var s = 0; s < studentChoices.length; s++) {
                        ...
                        if (studentChoice != null) {
                            ...
                            currentStepMultipleChoiceResponse += studentChoiceText;
                        }
                    }
                }
            }
        ...
        otherWork += 'Multiple Choice from current step: ';

        if (currentStepMultipleChoiceResponse == '') {
            otherWork += 'Empty';
        } else {
            otherWork += currentStepMultipleChoiceResponse;
        }
...
function saveWISE5State(componentState) {
    ...
    if (reuseComponentState == true) {
        if (latestComponentStateSaved != null) {
            componentState.id = latestComponentStateSaved.id;
        }
    }
...
function getStudentWork() {
    ...
    var message = {
        ...
        getStudentWorkParams: {
            getLatestStudentWorkFromThisComponent: true,
            getLatestStudentWorkFromThisNode: true,
            getLatestStudentWorkFromOtherComponents: true,
            otherComponents: [
                {
                    nodeId: "node54",
                    componentId: "1b630z8wa5"
                }
            ]
        }
    };
...
/**
 * Update a component state in the latestStudentWorkFromThisNode array
 * @param componentState the new component state
 */
function updateLatestStudentWorkFromThisNode(componentState) {
    
    if (latestStudentWorkFromThisNode != null) {
        
        // loop through all the component states from this node
        for (var c = 0; c < latestStudentWorkFromThisNode.length; c++) {
            var tempComponentState = latestStudentWorkFromThisNode[c];
            
            if (tempComponentState != null) {
                
                if (tempComponentState.nodeId == componentState.nodeId &&
                    tempComponentState.componentId == componentState.componentId) {
                    
                    // we have found the component state we want to update
                    
                    /*
                     * remove the old component state and replace it with the new 
                     * component state
                     */
                    latestStudentWorkFromThisNode.splice(c, 1, componentState);
                }
            }
        }
    }
...
function getStudentWorkByNodeIdAndComponentId(nodeId, componentId) {
    ...
    if (nodeId != null && componentId != null) {
        ...
        if (componentState == null && latestStudentWorkFromOtherComponents != null) {
            
            // loop through the component states from other nodes
            for (var c = 0; c < latestStudentWorkFromOtherComponents.length; c++) {
                
                // get a component state
                var tempComponentState = latestStudentWorkFromOtherComponents[c];
